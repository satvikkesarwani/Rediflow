...
import { QrCode, upiIntent } from '../components/QrCode';
  ...
  const handlePay = async () => {
    ...
    if (method === 'wallet') {
      try {
        ...
        if (e.status === 400) { setResult(e); setState('failed'); }
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: '24px', marginTop: 28, marginBottom: 20, boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid #f1f5f9' }}>
            ...
            <div style={{ display: 'flex', gap: 24, justifyContent: 'space-between' }}>
              <div style={{ textAlign: 'left' }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: '#0F172A' }}>₹{payable}</div>
        ...
        <div className="animate-scale-in" style={{ textAlign: 'center' }}>
          ...
          <div style={{ background: 'white', padding: '16px', borderRadius: 16, marginTop: 16, border: '1px solid #e2e8f0' }}>
            <p style={{ color: '#475569', fontSize: 14 }}>
              Required: <span style={{ fontWeight: 700 }}>₹{payable}</span><br />
  ...
  return (
    <div className="screen-enter" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: 'linear-gradient(160deg, #ecfdf5 0%, #d1fae5 100%)', padding: '18px 20px 22px', color: '#065f46' }}>
        <button onClick={onBack} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(6,95,70,0.1)', border: 'none', color: '#065f46', padding: '8px 14px', borderRadius: 20, cursor: 'pointer', fontSize: 13, fontWeight: 600, marginBottom: 16 }}>
      ...
      <div style={{ padding: '18px 18px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
        {/* Total */}
        ...
        <div style={{ background: 'white', borderRadius: 14, border: '1px solid #E2E8F0' }}>
          ...
          {showBreakdown && (
            <div className="animate-fade-in" style={{ padding: '0 16px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {discount > 0 && <Row label={`Coupon ${offer.code}`} value={`-₹${discount}`} green />}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <MethodTab active={method === 'wallet'} onClick={() => setMethod('wallet')} icon={<Wallet size={16} />} label="NCMC" />
        ...
        {method === 'upi' && (
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E2E8F0', padding: '20px', textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 12, fontSize: 12, color: '#94A3B8' }}>QR expires in {mmss}</div>
          </div>
        )}
      ...
      <div style={{ padding: '14px 18px 20px', background: 'white', borderTop: '1px solid #F1F5F9' }}>
        <button className="btn-primary" id="pay-btn" onClick={handlePay} disabled={state === 'paying'} style={{ borderRadius: 50 }}>
          {state === 'paying'
...
function Row({ label, value, bold, green }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
      ...
      <span style={{ fontSize: bold ? 15 : 13, fontWeight: bold ? 800 : 600, color: green ? '#059669' : '#0f172a' }}>{value}</span>
    </div>
  );
}
